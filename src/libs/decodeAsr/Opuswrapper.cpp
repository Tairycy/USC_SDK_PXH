...
#ifdef OPUS_SAVE_PCM
	file_log_pcm =  fopen("opus_log.pcm", "wb");
#endif
}

// 2014-03-05 zhangkaijun
void Opus::set_bitrate(int bitrate){
	if(bitrate < 16000) {
		bitrate = 16000;
	}
	...
	bitrate_16 = bitrate;
	set_mode(current_in_mode,current_out_mode);
}

void Opus::release_opus() {
...
int Opus::set_mode(int in_mode,int out_mode) {
	...
	int bitrate = (out_mode==WB_MODE)?bitrate_16:(bitrate_16>>1);
