...
unsigned int CBase64::CalculateRecquiredDecodeOutputBufferSize (char* p_pInputBufferString)
{
    ...
    for (int i=0;i<BufferLength;++i){
        printf("%c\t", p_pInputBufferString[i]);
    }
#endif
...
#if 0
    printf("++++++++++en base 64 qian++++++++++\n");
    printArray(p_pInputBuffer, p_InputBufferLength);
    printf("++++++++++++++++++++\n");
#endif
...
#if 0
    printf("````````````base 64 hou``````````\n");
    printArray(p_pOutputBufferString, strlen(p_pOutputBufferString));
    printf("````````````````````````\n");
#endif
}
...
unsigned int CBase64::DecodeBuffer (char* p_pInputBufferString, char* p_pOutputBuffer)
{
    ...
    char ByteQuartet [4];
#if 0
    printf("````````````de base 64 hou``````````\n");
    printArray(p_pInputBufferString, strlen(p_pInputBufferString));
    printf("````````````````````````\n");
#endif
...
#if 0
    printf("++++++++++base 64 qian++++++++++\n");
    printArray(p_pOutputBuffer, strlen(p_pInputBufferString));
    printf("++++++++++++++++++++\n");
#endif
...
#if 0
    printf("ENCODE input length %d, Calculate length %d\n", p_InputByteCount, Size-1);
#endif
...
#if 0
    printf("DECODE input length %d,  Calculate length %d\n", strlen(p_pInputBufferString), Size);
#endif
